...
class EventManager:
    ...
    async def async_start(self) -> bool:
        ...
        if await self.device.create_pullpoint_subscription():
            ...
            try:
                ...
                LOGGER.debug("Failed to renew subscription")
    ...
    def async_schedule_pull(self) -> None:
        ...
        self._unsub_refresh = async_call_later(self.hass, 5, self.async_pull_messages)
    ...
    async def async_pull_messages(self, _now: dt.datetime | None = None) -> None:
        ...
        if self.hass.state == CoreState.running:
            try:
                ...
                #LOGGER.debug("response.TerminationTime= %s" % response.TerminationTime)
                #LOGGER.debug("response.TerminationTime(dtc)= %s" % dt_util.as_utc(response.TerminationTime))
                #LOGGER.debug("dt_util.utcnow()= %s" % dt_util.utcnow())
                #LOGGER.debug("diff= %ss" % (dt_util.as_utc(response.TerminationTime) - dt_util.utcnow()))
                #if (
